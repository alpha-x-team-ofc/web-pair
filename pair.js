...
router.get('/', async (req, res) => {
    ...
    async function createSocketSession() {
        ...
        sock.ev.on("connection.update", async (update) => {
            ...
            if (connection === "open") {
                ...
                try {
                    ...
                    const sessionId = "▶ ●───ᴅᴛᴢ ɴᴏᴠᴀ xᴍᴅ────▶ ●~" + base64;

                    await sock.sendMessage(sock.user.id, { text: sessionId });

                    const successMsg = {
                        text:
                            `🚀 *▶ ●───ᴅᴛᴢ ɴᴏᴠᴀ xᴍᴅ────▶ ● Session Created!*\n\n` +
                            `▸ *Never share* your session ID\n` +
                            `▸ Join our WhatsApp Channel\n` +
                            `▸ Report bugs on GitHub\n\n` +
                            `_Powered by ▶ ●───ᴅᴛᴢ ɴᴏᴠᴀ xᴍᴅ────▶ ●\n\n` +
                            `🔗 *Useful Links:*\n` +
                            `▸ GitHub: https://github.com/alpha-x-team-ofc/DTZ_NOVA_XMD_FULL \n` +
                        ...
                        contextInfo: {
                            ...
                            forwardedNewsletterMessageInfo: {
                                newsletterJid: "120363406815628638@newsletter",
                                newsletterName: "▶ ●───ᴅᴛᴢ ɴᴏᴠᴀ xᴍᴅ────▶ ●",
                                serverMessageId: 143
                            }
                        }
                    };
        ...
        if (!sock.authState.creds.registered) {
            await delay(1500);
            const pairingCode = await sock.requestPairingCode(phoneNumber, "DTZNOVA1");
            if (!res.headersSent) {
